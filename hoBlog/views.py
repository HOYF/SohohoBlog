...
class IndexView(ListView):
    model = Post # 将model 指定为 Post ，告诉Django我要获取的模型是Post
...
class CategoryView(ListView):
    ...
    def get_queryset(self):
        # 先根据URL中捕获的分类id，并从数据库中获取分类
        cate = get_object_or_404(Category, pk=self.kwargs.get('pk'))
        # 然后使用 filter 函数过滤出该分类下的全部文章
        # 和IndexView 不同的地方是，我们覆写了父类的 get_queryset 方法，该方法默认获取指定模型的全部列表数据，
        # 为了获取指定分类下的文章列表数据，我们覆写该方法，改变他的默认行为
